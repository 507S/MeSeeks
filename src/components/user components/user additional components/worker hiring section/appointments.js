...
function Appointment() {
  ...
  const fetchProducts = () => {
    axios
      ...
      .then((res) => {
        ...
        setAllWork(res.data);
        axios
          .get(`http://localhost:8003/api/users/pending/${uid}`)
          .then((res) => {
            console.log(res.data);
            setAllPendingWork(res.data);
            axios
              .get(`http://localhost:8003/api/users/completed/${uid}`)
              .then((res) => {
                console.log(res.data);
  ...
  const handleComplete = (id) => {
    console.log("here at handle complete", id)
    try {
      axios.post(`http://localhost:8003/api/complete-work`, { id })
      window.location.reload()
    }
    catch (e) {
      alert(e)
            ...
            {allPendingWork.map((work, i) => (
              <div class="card mt-3 ">
                ...
                <div class="card-body">
                  ...
                  <button class="btn btn-danger" style={{ float: "right" }} onClick={() => handleComplete(work._id)} > <i className="fa-solid fa-trash-can"></i>&nbsp;Complete</button>
                </div>
              </div>
            ))}
          </div>
